...
function SidebarProvider({
	...
	const setOpen = useCallback(
		(value: boolean | ((value: boolean) => boolean)) => {
			...
			// biome-ignore lint/suspicious/noDocumentCookie: intentional cookie write for sidebar state persistence
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
	...
	return (
		<button
			...
			className={cn(
				"absolute inset-y-0 z-20 hidden `after:w-0.5 w-4 transition-all ease-linear after:absolute after:inset-s-1/2 after:inset-y-0 hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
